import { BookOpen, Briefcase, Camera, ChevronDown, ChevronRight, Clock, Code2, Coffee, File, FileText, Folder, FolderPlus, Grid2X2, Heart, Home, Image as ImageIcon, Info, Layers, LayoutList, MapPin, Music, Plus, Rows3, Star, Table, X } from 'lucide-react';
...
const SPACE_ICONS = { 'map-pin': MapPin, home: Home, 'book-open': BookOpen, briefcase: Briefcase, heart: Heart, star: Star, coffee: Coffee, music: Music, camera: Camera, code: Code2 } as const;
...
export function SpacesView({ workspaces, onCreate, onSelect }: { workspaces: WorkspaceListItem[]; onCreate: () => void; onSelect: (id: string) => void }) {
  ...
  useEffect(() => { void Promise.all(workspaces.map(async (workspace) => { try { const data = await window.aether.workspace.load(workspace.id); return [workspace.id, data.analyzedFiles.map((file) => fileIconForName(file.fileName).color)] as const; } catch { return [workspace.id, []] as const; } })).then((entries) => setFileColors(Object.fromEntries(entries))); }, [workspaces]);

  return <div className="h-full overflow-auto bg-[#F8F8FA] p-8"><h1 className="text-[22px] font-semibold text-[#29292D]">Spaces</h1><p className="mt-1 text-[13px] text-[#85858B]">Your saved canvases, locally stored.</p><div className="mt-6 grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-4">{workspaces.map((space) => { const Icon = SPACE_ICONS[space.icon as keyof typeof SPACE_ICONS] ?? MapPin; const colors = fileColors[space.id] ?? Array.from({ length: Math.min(space.fileCount, 9) }, () => space.iconColor); const dots = colors.slice(0, 9); const overflow = Math.max(space.fileCount - 9, 0); return <button className="h-[160px] rounded-[12px] border border-[#E5E3E1] bg-white p-4 text-left shadow-[0_2px_8px_rgba(0,0,0,0.06)] transition hover:-translate-y-0.5 hover:shadow-[0_8px_18px_rgba(0,0,0,0.09)]" key={space.id} onClick={() => onSelect(space.id)} type="button"><span className="grid h-10 w-10 place-items-center rounded-full text-white" style={{ backgroundColor: space.iconColor }}><Icon size={19} /></span><p className="mt-4 truncate text-[16px] font-semibold">{space.name}</p><p className="mt-1 text-[12px] text-[#8A8A90]">{space.fileCount} files · Updated {new Date(space.updatedAt).toLocaleDateString()}</p><div className="mt-4 flex h-4 items-center gap-1" title={`${space.fileCount} files`}>{overflow > 0 && <span className="mr-0.5 text-[10px] font-semibold text-[#77777D]">+{overflow}</span>}{dots.map((color, index) => <i className="h-2 w-2 rounded-full ring-2 ring-white" key={`${space.id}-${index}`} style={{ backgroundColor: color }} />)}</div></button>; })}<button className="grid h-[160px] place-items-center rounded-[12px] border border-dashed border-[#CFCBC7] text-[#88888E] transition hover:bg-white" onClick={onCreate} type="button"><span className="flex flex-col items-center gap-2"><Plus size={22} />Create new space</span></button></div></div>;
}
