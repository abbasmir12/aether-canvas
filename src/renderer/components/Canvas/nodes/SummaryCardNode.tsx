...
function Section({ children, index, relationshipType }: { children: React.ReactNode; index: number; relationshipType: 'dates' | 'cost' | 'tasks' | 'place' }) {
  ...
  return (
    ...
    >
      <Handle id={`summary-${relationshipType}`} className="!h-[6px] !w-[6px] !border !border-white/90 !shadow-[inset_0_0_0_1px_#fff]" position={Position.Left} style={{ backgroundColor: colors[relationshipType], top: '50%' }} type="target" />
      {children}
    </motion.section>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        ...
        {checklist && <Section index={sectionIndex++} relationshipType="tasks"><SectionHeading Icon={CheckSquare}>Packing</SectionHeading><div className="flex items-end justify-between"><p className="text-[12px] text-[#444449]"><b className="text-[22px] leading-none text-[#303034]">{packed}</b> / {packTotal} packed</p></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-[#E7E5E9]"><motion.div animate={{ width: `${packTotal ? (packed / packTotal) * 100 : 0}%` }} className="h-full rounded-full bg-[#9B72CF]" initial={{ width: 0 }} transition={{ delay: 2.4, duration: 0.45 }} /></div></Section>}
