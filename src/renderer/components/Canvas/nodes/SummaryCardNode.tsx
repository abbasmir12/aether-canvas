...
function CompactPreview({ module, color }: { module: DashboardModule; color: string }) {
  ...
  if (module.visual === 'ring-metric') return (
    <div className="mt-3 flex items-center gap-3.5">
      <div className="relative grid h-[62px] w-[62px] place-items-center rounded-full bg-[radial-gradient(circle_at_50%_45%,#FFFFFF_0%,#FBFBFA_62%,#F0EFEC_100%)] shadow-[inset_0_1px_2px_rgba(27,27,31,0.05)]">
        <svg className="absolute h-[62px] w-[62px] -rotate-90"><circle cx="31" cy="31" fill="none" r="25" stroke="#ECEBE8" strokeWidth="7" /><circle cx="31" cy="31" fill="none" r="25" stroke={color} strokeDasharray="117 157" strokeLinecap="round" strokeWidth="7" /></svg>
        <b className="text-center text-[10px] font-semibold tracking-[-0.04em] text-[#313137]">{compact.primary}</b>
      </div>
      <div className="min-w-0 space-y-1.5 text-[10px]"><p className="flex items-center gap-1.5 font-medium text-[#4E4E54]"><i className="h-2 w-2 rounded-full shadow-[0_1px_2px_rgba(0,0,0,0.16)]" style={{ backgroundColor: color }} />{compact.secondary}</p><p className="flex items-center gap-1.5 text-[#85848A]"><i className="h-2 w-2 rounded-full bg-[#C7C5C0]" />{compact.tertiary}</p></div>
    </div>
  );
...
function ModuleShell({ module, open, onToggle, onHover, children }: { module: DashboardModule; open: boolean; onToggle: () => void; onHover: (active: boolean) => void; children: React.ReactNode }) {
  ...
  return <motion.section layout className="group relative overflow-visible rounded-[13px] border border-[#E2E1E4] bg-[linear-gradient(145deg,#FFFFFF_0%,#FCFCFC_100%)] px-3.5 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,.96),0_1px_2px_rgba(25,25,29,.035)] transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-px hover:border-[#D7D5D9] hover:shadow-[inset_0_1px_0_rgba(255,255,255,1),0_7px_16px_rgba(25,25,29,.075)]" onMouseEnter={() => onHover(true)} onMouseLeave={() => onHover(false)} transition={{ duration: 0.22 }}>
...
export default function SummaryCardNode({ id, data, selected }: NodeProps<SummaryCardNodeType>) {
  ...
  return (
    <motion.article
      ...
      className={`relative w-[332px] overflow-visible rounded-[20px] border bg-[linear-gradient(145deg,#FFFFFF_0%,#FCFCFD_68%,#F9F9FA_100%)] p-4 shadow-[0_18px_42px_rgba(23,23,28,0.13),0_3px_8px_rgba(23,23,28,0.06),inset_0_1px_0_rgba(255,255,255,0.98)] ${selected ? 'border-[#4A90D9] ring-2 ring-[#4A90D9]/15' : 'border-[#D9D8DC]'}`}
      initial={{ opacity: 0, x: 80 }}
      transition={{ delay: data.assemblyDelay ?? 1.2, duration: 0.42, ease: [0.22, 1, 0.36, 1] }}
    >
      <div aria-hidden className="pointer-events-none absolute inset-x-5 top-0 h-px bg-gradient-to-r from-transparent via-white to-transparent" />
      <header className="relative mb-4 flex items-start gap-3 px-0.5 pt-0.5">
        <span className="grid h-11 w-11 shrink-0 place-items-center rounded-[13px] text-white shadow-[inset_0_1px_1px_rgba(255,255,255,.48),0_5px_12px_rgba(40,40,46,.16)]" style={{ background: `linear-gradient(145deg, ${headerColor}, ${headerColor}D4)` }}><Sparkles size={22} strokeWidth={2.15} /></span>
        <div className="min-w-0 flex-1 pt-0.5">
          {editing ? <input autoFocus className="nodrag w-full rounded-[7px] bg-[#F4F3F1] px-1.5 py-0.5 text-[22px] font-semibold tracking-[-0.045em] outline-none ring-2 ring-[#4A90D9]/30" onBlur={() => { if (title.trim()) emit('aether:rename-cluster', { summaryId: id, name: title.trim() }); setEditing(false); }} onChange={(event) => setTitle(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') event.currentTarget.blur(); if (event.key === 'Escape') setEditing(false); }} value={title} /> : <h2 className="truncate text-[22px] font-semibold leading-[1.05] tracking-[-0.05em] text-[#29292E]" onDoubleClick={() => setEditing(true)}>{plan.title || data.cluster.name}</h2>}
          <p className="mt-1.5 text-[11px] font-medium tracking-[-0.01em] text-[#77767D]">{plan.subtitle || data.cluster.dateRange}</p>
        </div>
      ...
      <div className="space-y-2.5">{plan.modules.map((module) => <ModuleShell key={module.id} module={module} onHover={(active) => focus(module, active)} onToggle={() => toggle(module)} open={openModule === module.id}>{content(module)}</ModuleShell>)}</div>
      <footer className="mt-4 flex justify-center border-t border-[#EEEDF0] pt-3.5"><button className="nodrag flex items-center gap-1.5 rounded-full border border-[#E4E2E4] bg-[linear-gradient(180deg,#FFFFFF,#F7F7F8)] px-3.5 py-1.5 text-[10px] font-medium text-[#696970] shadow-[inset_0_1px_0_rgba(255,255,255,.95),0_1px_2px_rgba(25,25,30,.04)] transition hover:-translate-y-px hover:border-[#D6D5D9] hover:shadow-[0_4px_9px_rgba(25,25,30,.08)]" onClick={() => emit('aether:highlight-sources', data.files.map((file) => file.id))} type="button"><SparklesIcon className="text-[#8A68AF]" size={13} />Generated from {data.files.length} files</button></footer>
    </motion.article>
  );
}
