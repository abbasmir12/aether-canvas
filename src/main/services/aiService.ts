    ...
    input: [
      {
        ...
        content: [
          {
            type: 'input_text',
            text: `You are the relationship and workspace compiler for Aether Canvas. Identify only meaningful semantic relationships grounded in the analyzed metadata. Prefer one strongest relationship type for each related file pair. Do not connect unrelated files.\n\nThen design the dashboard this exact cluster needs. You must preserve Aether's Tokyo Trip dashboard quality and hierarchy while adapting it to the actual domain. This is the full visual/component grammar to use as inspiration:\n\n- Header: one large colored Lucide icon at left, a strong 22px workspace title, one muted date/status subtitle, and a three-dot overflow menu at right.\n- Each module: white 12px-rounded inset panel with a colored circular Lucide icon, concise section label, and a small semantic connection port on its left edge.\n- Journey/Timeline module: a clear horizontal route or event rail; compact origin → travel/action icon → destination or milestone; supporting date/time metadata and a secondary check-in/next-step detail.\n- Budget module: a large central ring/progress visualization; bold amount in its center; two-item colored legend for tracked/spent and remaining; precise values aligned on the right.\n- Packing/Tasks module: oversized completion fraction, action label, and a full-width colored progress bar.\n- Map/Places module: a bounded mini-map with prominent pins and concise location labels.\n- Footer: a small rounded 'Generated from N files' provenance pill with a sparkle.\n\nThe dashboard is not travel-only. Translate these same interaction patterns to the user's subject: for study use concepts/progress/resources; for work use milestones/tasks/budget; for health use results/timeline/actions; for recipes use ingredients/checklist/timing. Keep the visual hierarchy rich, calm, and Apple-quality—not generic panels.\n\nChoose 2-5 modules based only on the files. Allowed kinds: overview, timeline, budget, checklist, map, tasks, topics, resources, results. Choose headerIcon and each module icon from: sparkles, plane, wallet, check-square, map, list-checks, book-open, file-text. Choose headerAccent and module accent from: dates (blue), cost (green), place (coral), tasks (purple), neutral (soft violet). Every module must have a specific short title, concise grounded summary, visual icon/accent, and exact contributing source file IDs. Return dashboard null only when the files cannot form a coherent workspace.\n\nFiles: ${JSON.stringify(files)}`,
          },
        ],
      },
    ],
