...
- The real map is presented as “Aether Atlas”: animated source and AI markers, custom focus behavior, semantic legend, location rail, premium popovers, integrated controls, and compliant attribution turn a default basemap into a coherent mini-app surface without requiring another API key.
- Aether Atlas preserves user pan and zoom after initialization and exposes actionable place cards on hover, including source provenance and direct directions links—interaction quality verified through the Electron smoke harness.
...
- Settings is a real control surface rather than a mock: users can switch between signature Liquid Glass ribbons and lightweight semantic lines, tune the canvas and minimap, control live sync, choose the GPT-5.6 runtime model and reasoning effort, securely supply a key, and test the configured connection.
- The custom canvas overview preserves Aether's visual grammar at miniature scale—source cards, category hubs, dashboard modules, visual answers, semantic ribbons, query traces, and the live viewport are all represented and hard-clipped to a premium navigable frame.
- Help is a searchable product guide with workflow education, supported-source guidance, keyboard shortcuts, explainability concepts, and a demo-ready living-files walkthrough.
- The human-designed Aether mark now forms one coherent identity across custom window chrome, first-run onboarding, Settings, Help, metadata, and packaged application icons; functional controls retain Lucide icons so branding never compromises usability.
...
- The Budget module demonstrates grounded mini-app behavior rather than decorative analytics: sample-file totals reconcile to `$2,200 planned / $1,460 spent / $740 available`, while the expanded ledger exposes category variance, editable actuals, saved state, category creation, and CSV export. Legacy cached analyses retain their authoritative aggregate instead of displaying a misleading `$0 left` state.
