...
- Ribbon refinement: implemented path-length-derived, staggered flow dots and a three-layer SVG river (blurred atmosphere, translucent main body, delayed white shimmer). Hub-to-summary trunks are deliberately wider and more opaque than their tributaries.
...
- Final connector architecture: replaced full-width per-file bands with thin, vertically varied tendrils converging at invisible category merge anchors; one shared river continues into each hub. A live Electron capture verified this reduces visual noise and produces the calm tributary-to-river pattern from the reference.
